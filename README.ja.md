...
**注意**: list出力の番号は他のコマンド（update、show、unregister）でも使用できます。

### リポジトリの詳細表示

特定のリポジトリの詳細情報を表示します。引数なしの場合は、すべてのリポジトリの概要を表示します：
...
showコマンドは以下を表示します：
- 引数なし：すべてのリポジトリのステータス概要
- 引数あり：リポジトリ情報（URL、ステータス、タイプ）
...
登録済みのリポジトリを登録解除します。登録解除時に、デプロイ済みファイルを削除するかどうかを選択できます：
...
# 確認なしで登録解除（デプロイ済みファイルは保持）
ccpm unregister owner/repo --force
```
...
| コマンド | レジストリ登録 | Gitリポジトリ | デプロイ済みファイル |
|---------|---------------|--------------|-------------------|
| `update` | 保持 | 更新 (git pull) | 再デプロイ (プロンプトあり) |
| `update --install` | 保持 | 更新 (git pull) | 自動デプロイ |
| `install` | 保持 | 変更なし | デプロイ (プロンプトあり) |
| `uninstall` | 保持 | 変更なし | 削除 |
| `unregister` | 削除 | 変更なし | 保持 (またはプロンプトで削除) |

### 各コマンドをいつ使うか：

- **`update`**: リポジトリから最新の変更を取得し、オプションでデプロイ
- **`update --install`**: 最新の変更を取得し、自動的にファイルをデプロイ
- **`install`**: 更新せずに現在のリポジトリ状態からファイルをデプロイ
- **`uninstall`**: デプロイ済みファイルを削除するが、リポジトリは登録されたまま
- **`unregister`**: リポジトリの追跡を停止（デプロイ済みファイルの削除オプション付き）
